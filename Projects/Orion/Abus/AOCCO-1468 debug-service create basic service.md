...
### 8/01/2024 Monday
Людер сказал, что нужно добавить авторизацию к debug endpoint-ам. Смотрю, как это сделано в других сервисах и добавляю. Завтра выходит Александр, посмотрим, что еще он скажет по этому поводу.
Кроме этого, работаю над subsystem тестами - нужно чтобы состояние БД сохранялось перед началом тестов и восстанавливалось по их завершению. Этот функционал я вроде реализовал, но появилась проблема с удалением записей из таблицы t_auth_users, полезла ошибка, что от нее зависит таблица t_person и из-за этого ее тоже нужно чистить. Александр ничего про это не говорил и нигде в задаче об этом не говорится. Более того, раньше этой ошибки не было. Где и когда я что сломал не понятно.
